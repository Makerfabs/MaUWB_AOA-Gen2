...
void twr_uwb_process(rx_pckt_t *pRxPckt, twr_info_t *pTwrInfo)
{
	...
	if(pTwrInfo->twr_tag.general.is_bind == true)
	{
		...
		if(fCode == TWR_RESP_MSG_FN && pRxPckt->rxDataLen == sizeof(resp_msg_t))
		{
			...
			{
				port_tx_msg("resp A0\n", strlen("resp A0\n"));
//				tag_received_timeout_set(pTwrInfo, 3*pTwrInfo->twr_tag.env.tag_respRxDelay_us);
			}
			else if(pRxMsg->mac.sourceAddr == 1)
			{
				port_tx_msg("resp A1\n", strlen("resp A0\n"));
//				tag_received_timeout_set(pTwrInfo, 2*pTwrInfo->twr_tag.env.tag_respRxDelay_us);
			}
			else if(pRxMsg->mac.sourceAddr == 2)
			{
				port_tx_msg("resp A2\n", strlen("resp A0\n"));
//				tag_received_timeout_set(pTwrInfo, 1*pTwrInfo->twr_tag.env.tag_respRxDelay_us);
			}
...
static void rx_to_cb(const ose_cb_data_t* cb_data)
{
	...
	{
		pTwrInfo->testAppState = (pTwrInfo->twr_tag.measure.rxResponseMask == 0)?(TA_TXE_WAIT):(TA_TXFINAL_WAIT_SEND);
		//【2.2场景】/【2.3场景】
						...
						case Twr_Tx_Blink_Sent:
							{
								...
								tag_received_timeout_set(pTwrInfo, RC_RX_TIMEOUT_US+1000);
							}
							break;
						case Twr_Tx_Poll_Sent:
							{
								...
								tag_received_timeout_set(pTwrInfo, pTwrInfo->twr_tag.env.tag_respRxDelay_us); 
							}
							break;

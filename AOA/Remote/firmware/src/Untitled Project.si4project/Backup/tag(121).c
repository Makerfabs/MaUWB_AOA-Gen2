...
void tag_received_timeout_set(twr_info_t *pTwrInfo, bool error)
{
	...
	sprintf(buf, "tag_received_timeout_set:%d, :%d\n", pTwrInfo->twr_tag.measure.remainingRespToRx, get_1us_tick_value());
	...
	uint32_t tmp = (pTwrInfo->twr_tag.env.tag_respRxDelay_us) * (MAX_ANCHOR_LIST_SIZE - pTwrInfo->twr_tag.measure.remainingRespToRx);
	uint64_t resp_rx_time = ose_merge_timestamp(pTwrInfo->txPcktBuf.buf.t_timeStamp, tmp - 100);	//50us
	ose_settrxdelay(OSE_RX_DELAY_M, resp_rx_time);											//2us
	ose_settrxdelay(OSE_RX_TIMEOUT_M,  pTwrInfo->twr_tag.env.tag_respRxTimeout_us);
	int16_t state = ose_rxenable(OSE_START_RX_DELAYED, 0);
}
...
static void tx_ok_cb(const ose_cb_data_t* cb_data)
{
	...
	if(pTxPckt->txState == Twr_Tx_Blink_Sent)
	{
		pTxPckt->t_timeStamp = (cb_data->timemask.tx_done_time == 1)?(cb_data->tx_done_time):(0);
	}
	else if(pTxPckt->txState == Twr_Tx_Poll_Sent)
	{
		...
		case TA_TXPOLL_WAIT_SEND:
			{
				...
				// 立即发送[使用滴答时钟监测,消耗1毫秒内]
				int16_t state = ose_starttx(OSE_START_TX_IMMEDIATE,(uint8_t*)&pTxPckt->arr, sizeof(poll_msg_t), 50);
				if(state == UWB_TX_OK)
				{
					pTwrInfo->testAppState = TA_TX_WAIT_CONF;
				}
				...
				if(pTwrInfo->txPcktBuf.buf.is_send == true)
				{
					...
					switch(pTwrInfo->txPcktBuf.buf.txState)
					{
						case Twr_Tx_Blink_Sent:
							{
								pTwrInfo->testAppState = TA_RX_WAIT_DATA;					//必须优于ose_rxenable
						...
						case Twr_Tx_Poll_Sent:
							{
								pTwrInfo->testAppState = TA_RX_WAIT_DATA;					//必须优于ose_rxenable
								uint32_t time_s = get_1us_tick_value( );
								uint64_t resp_rx_time = ose_merge_timestamp(pTwrInfo->txPcktBuf.buf.t_timeStamp, pTwrInfo->twr_tag.env.tag_respRxDelay_us - 100);	//50us
								ose_settrxdelay(OSE_RX_DELAY_M, resp_rx_time);											//2us
								ose_settrxdelay(OSE_RX_TIMEOUT_M,  pTwrInfo->twr_tag.env.tag_respRxTimeout_us);
								int16_t state = ose_rxenable(OSE_START_RX_DELAYED, 0);
							}
...
void instance_run(void)
{
	...
	//	pTwrInfo->twr_tag.machine_period.nextWakeUpTime_ms = pTwrInfo->twr_tag.machine_period.preWakeUpTime_ms +  pTwrInfo->twr_tag.env.sframePeriod_ms/* + pTwrInfo->twr_tag.machine_period.interval_in_ms*/;
		pTwrInfo->twr_tag.machine_period.nextWakeUpTime_ms = pTwrInfo->twr_tag.machine_period.preWakeUpTime_ms +  1000/* + pTwrInfo->twr_tag.machine_period.interval_in_ms*/;
		pTwrInfo->twr_tag.machine_period.timeron = true;
		pTwrInfo->twr_tag.measure.slotCorr_ms = 0;
//		_dbg_printf("nextWakeUpTime_ms:%d, portGetTickCnt:%d\n", pTwrInfo->twr_tag.machine_period.nextWakeUpTime_ms, portGetTickCnt());
	}

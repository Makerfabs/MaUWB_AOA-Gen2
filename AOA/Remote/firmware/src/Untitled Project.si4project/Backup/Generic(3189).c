...
void App_Module_sys_startup_check()
{
	...
	{
		HalIWDG_Feed();
		uint8_t CHRG = GPIO_ReadInputDataBit(HAL_KEY_CHRG_PORT, HAL_KEY_CHRG_PIN);		//充电IO口
...
void App_Module_Sys_Deal_IO_LED_Event(void)
{
	...
	int  period = (sys_state.para_tag.tag_detail_para.mode == 0)?(500):(1000);
...
{
	sys_state.para_tag.tag_detail_para.turn_up = (GPIO_ReadInputDataBit(HAL_KEY1_Up_PORT, HAL_KEY1_Up_PIN) == 1)?(false):(true) ;
	sys_state.para_tag.tag_detail_para.turn_left = (GPIO_ReadInputDataBit(HAL_KEY2_Left_PORT, HAL_KEY2_Left_PIN) == 1)?(false):(true) ;
	sys_state.para_tag.tag_detail_para.turn_right = (GPIO_ReadInputDataBit(HAL_KEY4_Right_PORT, HAL_KEY4_Right_PIN) == 1)?(false):(true) ;
	sys_state.para_tag.tag_detail_para.turn_down = (GPIO_ReadInputDataBit(HAL_KEY5_Down_PORT, HAL_KEY5_Down_PIN) == 1)?(false):(true) ;
	...
	if(sys_state.system_turnoff == true && HalKey_Loose_Judge() && sys_state.system_DoneState != Sys_DoneState_DeepSleep)
	{
		...
		while(true)
		{
			...
			{
				HalPmu_Enter(0xffff, false);		//10秒唤醒喂狗
			}
		}
	}
}
...
void App_Moudle_Device_SetMode(int key_sn)
{
	...
	// 遥控器(KEY6)
	sys_state.para_tag.tag_detail_para.mode = (key_sn == HAL_KEY6)?(1):(0);
	_dbg_printf("Mode:%d, testAppState:%d, done:%d, event:%d\n", 
		sys_state.para_tag.tag_detail_para.mode, 
		pTwrInfo->testAppState, 
		pTwrInfo->done,
		pTwrInfo->twr_tag.machine_period.event);
}
...
tag_detail_para_t App_Module_Get_SysState_Sensor(void)
{
	...
	sensor.turn_up       = sys_state.para_tag.tag_detail_para.turn_up;			//(遥控专用)向前
	sensor.turn_down     = sys_state.para_tag.tag_detail_para.turn_down;		//(遥控专用)向后
	sensor.turn_left     = sys_state.para_tag.tag_detail_para.turn_left;		//(遥控专用)向左
	sensor.turn_right    = sys_state.para_tag.tag_detail_para.turn_right;		//(遥控专用)向右
	sensor.mode          = sys_state.para_tag.tag_detail_para.mode;				//(遥控专用)模式
	sensor.recal         = sys_state.para_tag.tag_detail_para.recal;			//(遥控专用)召回
	sensor.lock          = sys_state.para_tag.tag_detail_para.lock;				//(遥控专用)上锁

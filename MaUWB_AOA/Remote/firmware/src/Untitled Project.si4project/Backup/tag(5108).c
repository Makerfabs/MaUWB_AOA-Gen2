...
void tag_received_timeout_set(twr_info_t *pTwrInfo, bool error)
{
	...
	int16_t state = ose_rxenable(OSE_START_RX_DELAYED|OSE_IDLE_ON_DLY_ERR, 0);
...
//					port_tx_msg(buf, strlen(buf));
					...
					test_mid = time_m - time_s;
					test_all = time_e - time_s;
					pTwrInfo->testAppState = TA_TX_WAIT_CONF;
				}
				else
				{
...
//								port_tx_msg("Twr_Tx_Poll_Sent\n", strlen("Twr_Tx_Poll_Sent\n"));
								...
								pTwrInfo->testAppState = TA_RX_WAIT_DATA;					//必须优于ose_rxenable
								uint32_t time_s = get_1us_tick_value();
								uint64_t resp_rx_time = ose_merge_timestamp(pTwrInfo->txPcktBuf.buf.t_timeStamp, 1400*1-300);	//50us
								ose_settrxdelay(OSE_RX_DELAY_M, resp_rx_time);											//2us
								ose_settrxdelay(OSE_RX_TIMEOUT_M, 1200);
								int16_t state = ose_rxenable(OSE_START_RX_DELAYED|OSE_IDLE_ON_DLY_ERR, 0);
								uint32_t time_e= get_1us_tick_value();

...
//				port_irq_uwb_chip(true);

				port_en_uwb_chip(true);
					sleep_ms(10);
#ifndef USE_UWB_ROM_VER
					download_firmware();
#endif
					uwb_start();
...
//								port_tx_msg(buf, strlen(buf));  
		...
		case TA_TXE_WAIT:
			{
#if (DEEP_SLEEP == 1 && WORK_SLEEP == 1)
				//bool state = ose_entersleep(OSE_SLEEP_DPD);
//				bool state = ose_entersleep(UWB_SLEEP_MODE);
//				port_irq_uwb_chip(false);	//停止中断
...
#if (DEEP_SLEEP == 1 && WORK_SLEEP == 1)
	ose_configuresleep(0, WAKEUP_METHOD_SLEEP_EN | WAKEUP_METHOD_CS, 0);
#endif

		...
		case TA_TXBLINK_WAIT_SEND :
			{
				...
				// 优先中断,在这边循环结束
				if(ose_starttx(OSE_START_TX_IMMEDIATE,(uint8_t*)&pTxPckt->arr, sizeof(blink_msg_t), 1000) == UWB_TX_OK)
				{
		...
		case TA_TXPOLL_WAIT_SEND:
			{
				...
				if(ose_starttx(OSE_START_TX_IMMEDIATE,(uint8_t*)&pTxPckt->arr, sizeof(poll_msg_t), 1000) == UWB_TX_OK)
				{
					pTwrInfo->testAppState = TA_TX_WAIT_CONF;
				}
				else

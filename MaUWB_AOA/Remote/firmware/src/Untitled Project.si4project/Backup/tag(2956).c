...
void tag_received_timeout_set(twr_info_t *pTwrInfo, bool error)
{
	...
	{
		port_tx_msg("timeout_set\n", strlen("timeout_set\n"));
		timeout_us = TAG_BLINK_TX_RC_RX_US+1000;
	}
	else
	{
		timeout_us = pTwrInfo->twr_tag.env.tag_respRxDelay_us;
	...
	if(state == UWB_HCI_FAIL)
	{
		ose_settrxdelay(OSE_RX_TIMEOUT_M,  timeout_us);
		int16_t state = ose_rxenable(OSE_START_RX_IMMEDIATE, 0);
		_dbg_printf("UWB_HCI_FAIL-1:%d\n", state);
		if(state == UWB_HCI_FAIL)
		{
			_dbg_printf("UWB_HCI_FAIL-2:%d\n", state);
...
void prepare_twr_final_msg(tx_pckt_t *pTxPckt, twr_info_t *pTwrInfo)
{
	// 填充final数据
	{
		...
		pTxMsg->final.motorAngle          = pTwrInfo->twr_tag.measure.motorAngle;
		pTxMsg->final.t2a_sensor          = App_Module_Get_SysState_Sensor();
	}
		...
		if(fCode == TWR_RESP_MSG_FN && pRxPckt->rxDataLen == sizeof(resp_msg_t))
		{
				...
				pRxMsg->resp.rNum == pTwrInfo->twr_tag.measure.twr_sn)
			{
				...
				if(pRxMsg->mac.sourceAddr == GATEWAY_ADDR){
					...
					pTwrInfo->twr_tag.measure.motorAngle = pRxMsg->resp.motorAngle;
				}
		...
		case TA_SLEEP_DONE :
			{
				...
				port_wakeup_uwb_chip();

				test_err = false;
				// 唤醒失败消耗800ms
				while(u1_wakeup_init() != 0)
				{
					test_err = true;
					HalDelay_nMs(5);
					_dbg_printf("u1 Init fault:%d\n", portGetTickCnt() - dwWakeup_starttime);
				}
				...
				if(test_err == true)
				{
					...
					sprintf(buf, "wakeup success %04X, dwWakeUpTimeCorr_ms:%d\n",ose_readdevid(), pTwrInfo->twr_tag.machine_period.dwWakeUpTimeCorr_ms);
		...
		case TA_TXBLINK_WAIT_SEND :
			{
				...
				if(state == UWB_TX_OK)
				{
					...
					pTwrInfo->testAppState = TA_TXE_WAIT;//TA_TX_WAIT_CONF;
				}
				else
				{
					//【1.1场景】
		...
		case TA_TXFINAL_WAIT_SEND:
			{
				...
				ose_settrxdelay(OSE_TX_DELAY_M, ((final_msg_t *)pTxPckt->arr)->final.finalTx_ts);													//2us
				if(ose_starttx(OSE_START_TX_DELAYED, (uint8_t *)&pTwrInfo->txPcktBuf.buf.arr, pTwrInfo->txPcktBuf.buf.txDataLen, 10) == UWB_TX_OK)	//占用863us
				{
				...
				if(pTwrInfo->txPcktBuf.buf.is_send == true)
				{
					...
					switch(pTwrInfo->txPcktBuf.buf.txState)
					{
						...
						case Twr_Tx_Poll_Sent:
							{
								pTwrInfo->twr_tag.measure.remainingRespToRx = MAX_ANCHOR_LIST_SIZE-1;
								pTwrInfo->twr_tag.measure.rxResponseMask = 0;
								pTwrInfo->twr_tag.measure.twr_timestamp.tagPollTxTime = pTwrInfo->txPcktBuf.buf.t_timeStamp;
								tag_received_timeout_set(pTwrInfo, false); 
							}
							break;
		...
		case TA_TXE_WAIT:
			{
				...
				// DPD
				disable_ose_irq();
				bool state = ose_entersleep(UWB_SLEEP_MODE);

				pTwrInfo->twr_tag.general.dw_sleep = true;

				_dbg_printf("TA_TXE_WAIT, U1 DeepSleep:%d\n", state);
#endif
...
void instance_run(void)
{
	...
	{
		pTwrInfo->twr_tag.env.sframePeriod_ms = 40;
		...
		pTwrInfo->twr_tag.machine_period.interval_in_ms = 50;
		...
		// ②全速工作时使用
		pTwrInfo->twr_tag.machine_period.nextWakeUpTime_ms = pTwrInfo->twr_tag.machine_period.preWakeUpTime_ms + pTwrInfo->twr_tag.machine_period.interval_in_ms;
	...
	if(pTwrInfo->twr_tag.machine_period.timeron == true)
	{
		...
		{
//			pTwrInfo->twr_tag.machine_period.interval_in_ms = 15000;
			HalPmu_Enter(pTwrInfo->twr_tag.machine_period.interval_in_ms, false);
			pTwrInfo->twr_tag.machine_period.event = DWT_SIG_SLEEP_TIMEOUT;
			_dbg_printf("rtc wakeup\n");
		}
...
//		_dbg_printf("Failed to initialize UWB %s at %d line.\r\n", __FUNCTION__, __LINE__);
	...
	if (ose_get_txrx_para(&trx_para, GET_FROM_ROM, 0x06) == OSE_ERROR) {
		_dbg_printf("e2\n");
//		_dbg_printf("Failed to get configuration parameters %s at %d line.\r\n", __FUNCTION__, __LINE__);
	...
	config_txrx_para.param.rf_params.tx_power = 120;
...
//		_dbg_printf("UWB configure failed %s at %d line.\r\n", __FUNCTION__, __LINE__);
	...
	ose_set_tx_rx_antennadelay(sys_para.param_Config.s.baseConfig.antTx, sys_para.param_Config.s.baseConfig.antRx); 	// 设置天线接受&发送延时
	ose_set_device_info(OSE_DEVICE_INFO_ALL, 0, 0, 0);
	ose_configureframefilter(OSE_FF_DISABLE,OSE_FF_DATA_EN|OSE_FF_PANID_EN);

	/* Config sleep parameters */
#if (DEEP_SLEEP == 1 && WORK_SLEEP == 1)
	ose_configuresleep(0, WAKEUP_METHOD_SLEEP_EN | WAKEUP_METHOD_CS | WAKEUP_METHOD_WK, 0);
#endif

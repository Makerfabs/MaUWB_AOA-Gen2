...
#endif
			for(int key = HAL_KEY1; key < HAL_KEYCNT; key++)
			{
...
//			sprintf(buf, "key_sn:%d\n", hal_key.key_sn);
//			port_tx_msg(buf, strlen(buf));
			_dbg_printf("key_sn:%d\n", hal_key.key_sn);
		}
		break;
		...
		case HAL_KEY_MODE_PRESS_DOUBE_JUDGE:
		{
			...
			_dbg_printf("double judge:%d, %d, %d\n", hal_key.key_sn, (timer - hal_key.loose_timer), hal_key.dcd_timer);
...
//				App_Moudle_Device_TurnOff();
	...
	if(key_judge == true)
	{
		uint8_t buf[32];
		memset(buf, 0, sizeof(buf));
		_dbg_printf("key init\n");
//		sprintf(buf, "key init\n");
//		port_tx_msg(buf, strlen(buf));

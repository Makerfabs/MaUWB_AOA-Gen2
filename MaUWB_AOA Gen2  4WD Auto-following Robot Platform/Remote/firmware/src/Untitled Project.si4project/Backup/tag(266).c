...
//	sprintf(buf, "state:%d,%d,%d\n", state, tmp, pTwrInfo->twr_tag.env.tag_respRxTimeout_us);
...
//							pTwrInfo->testAppState = TA_RX_WAIT_DATA;					//必须优于ose_rxenable
								...
								pTwrInfo->testAppState = TA_RX_WAIT_DATA;					//必须优于ose_rxenable
								uint32_t time_s = get_1us_tick_value( );
								uint64_t resp_rx_time = ose_merge_timestamp(pTwrInfo->txPcktBuf.buf.t_timeStamp, 1200-100);;	//50us
								ose_settrxdelay(OSE_RX_DELAY_M, resp_rx_time);											//2us
								ose_settrxdelay(OSE_RX_TIMEOUT_M, 1000);
								int16_t state = ose_rxenable(OSE_START_RX_DELAYED, 0);
								uint32_t time_e= get_1us_tick_value();
								...
								uint8_t buf[64];
								memset(buf, 0, sizeof(buf));
								sprintf(buf, "Twr_Tx_Poll_Sent state:%d\n", state);
								port_tx_msg(buf, strlen(buf));
#endif

 ...
 *
 */
void tag_received_timeout_set(twr_info_t *pTwrInfo, bool error)
{
//	{
//		pTwrInfo->twr_tag.measure.remainingRespToRx--;
//	}

//	if(pTwrInfo->twr_tag.measure.remainingRespToRx < 0)
//	{
//		// 注①：如果为最后一次打开接受且超时，则发送final数据包切换为发送Final
//		if(error == true){pTwrInfo->testAppState = TA_TXFINAL_WAIT_SEND;}
//		return ;
//	}
//	
//	uint64_t  tmp64 = pTwrInfo->twr_tag.measure.twr_timestamp.tagPollTxTime;
//	tmp64 += util_us_to_dev_time((pTwrInfo->twr_tag.env.tag_respRxDelay_us) * (MAX_ANCHOR_LIST_SIZE - pTwrInfo->twr_tag.measure.remainingRespToRx));
//	tmp64 &= MASK_TXDTS;         

//	uint16_t timeout = (uint32_t)util_us_to_sy(pTwrInfo->twr_tag.env.tag_respRxTimeout_us + 50);

//	dwt_setdelayedtrxtime(tmp64 >> 8);			//设置延迟接受时间
//	dwt_setrxtimeout(timeout);					//设置接受超时时间
//	dwt_rxenable(DWT_START_RX_DELAYED); 		//延迟接受，如果开启失败则立即开启
...
{
//	uint64_t tmp64 = pTwrInfo->twr_tag.measure.twr_timestamp.tagPollTxTime;
//	uint32_t tmp  = (pTwrInfo->twr_tag.env.tag_pollTxFinalTx_us);
...
//		pTxMsg->final.finalTx_ts          = (app.pConfig->s.baseConfig.antTx + tmp64) & MASK_40BIT;
...
//	pTxPckt->is_send                = false;
//	pTxPckt->seqNum                 = (pTxPckt->seqNum+1);
//	pTxPckt->txFlag                 = (DWT_START_TX_DELAYED);
//	pTxPckt->delayedTxTimeH_sy      = tmp64 >>8;
//	pTxPckt->delayedRxTime_sy       = 0;
//	pTxPckt->delayedRxTimeout_sy    = 0;
}
	...
	else
	{
		...
		if(fCode == TWR_RNGCFG_MSG_FN && pRxPckt->rxDataLen == sizeof(rngcfg_msg_t))
		{
			...
			if(pRxMsg->rngcfg.tagId == pTwrInfo->eui64)
			{
				...
				pTwrInfo->twr_tag.env.tag_respRxTimeout_us  = 0;//pTwrInfo->twr_tag.msg_time.response.us;
				...
				_dbg_printf("标签绑定成功, 短地址:%04X, 超级帧周期:%d,%d, 时间校准:%d, tag_respRxTimeout_us:%d, tag_respRxDelay_us:%d, poll.preamble_us:%d\n", 
...
static void tx_ok_cb(const ose_cb_data_t* cb_data)
{
	...
	pTxPckt->t_timeStamp = (cb_data->timemask.tx_done_time == 1)?(cb_data->tx_done_time):(0);
	pTxPckt->is_send = true;
//	_dbg_printf("tx_ok_cb\n");
}
...
static void rx_ok_cb(const ose_cb_data_t* cb_data)
{
	...
	pRxPckt->r_timeStamp    = (cb_data->timemask.rx_ok_time == 1)?(cb_data->rx_ok_time):(0);
...
//#endif
		...
		case TA_TXBLINK_WAIT_SEND :
			{
				_dbg_printf("send blink\n");
				tx_pckt_t *pTxPckt = &pTwrInfo->txPcktBuf.buf;

				uint32_t time_s = portGetTickCnt();
				prepare_twr_blink_msg(pTxPckt, pTwrInfo);
				// 立即发送[使用滴答时钟监测,消耗1毫秒内]
				if(ose_starttx(OSE_START_TX_IMMEDIATE,(uint8_t*)&pTxPckt->arr, sizeof(blink_msg_t), 50) == UWB_TX_OK)
				{
		...
		case TA_TXPOLL_WAIT_SEND:
			{
				...
				// 立即发送[使用滴答时钟监测,消耗1毫秒内]
				if(ose_starttx(OSE_START_TX_IMMEDIATE,(uint8_t*)&pTxPckt->arr, sizeof(poll_msg_t), 100) == UWB_TX_OK)
				{
					pTwrInfo->testAppState = TA_TX_WAIT_CONF;
				}
				else
				{
					//【2.1场景】
					pTwrInfo->testAppState = TA_TXE_WAIT;
					port_tx_msg("send poll fault\n", strlen("send poll fault\n"));
				}
...
//				{
//					_dbg_printf("Invalid rxResonseMask:%d\n", pTwrInfo->twr_tag.measure.twr_sn);
...
//				error_e ret = tx_start(pTxPckt);
//				if(ret == DWT_SUCCESS)
//				{
//					//【3.2场景】
//					pTwrInfo->testAppState = TA_TX_WAIT_CONF;
////					_dbg_printf("final send ok\n");
//				}
//				else
//				{
//					//【3.1场景】
...
//								ose_settrxdelay(OSE_RX_DELAY_M, TAG_BLINK_TX_RC_RX_US);		//设置延迟接受时间
//								ose_settrxdelay(OSE_RX_TIMEOUT_M, RX_FIXED_TIMEOUT_US);		//设置接受超时时间
								uint64_t rngcfg_rx_time = ose_merge_timestamp(pTwrInfo->txPcktBuf.buf.t_timeStamp, TAG_BLINK_TX_RC_RX_US-100);;	//50us
								ose_settrxdelay(OSE_RX_DELAY_M, rngcfg_rx_time);											//2us
								ose_settrxdelay(OSE_RX_TIMEOUT_M, 800);
								int16_t state = ose_rxenable(OSE_START_RX_DELAYED, 1000);
								uint32_t time_e= get_1us_tick_value();

								_dbg_printf("state:%d, time:%d\n",state, time_e - time_s);
								pTwrInfo->testAppState = TA_RX_WAIT_DATA;
							}
...
//				pTwrInfo->testAppState = TA_SLEEP_DONE;
				pTwrInfo->testAppState = TA_TXBLINK_WAIT_SEND;
...
//		// 充电时，stm32不休眠
//		if(sys_state.sys_work_mode >= Sys_Operate_Mode_USB_CONNET || 

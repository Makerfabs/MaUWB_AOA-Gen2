...
void tag_received_timeout_set(twr_info_t *pTwrInfo, bool error)
{
	...
	sprintf(buf, "tag_received_timeout_set:%d, :%d :%d :%d\n", 
				...
				if(pTwrInfo->txPcktBuf.buf.is_send == true)
				{
					...
					switch(pTwrInfo->txPcktBuf.buf.txState)
					{
						case Twr_Tx_Blink_Sent:
							{
								...
								ose_settrxdelay(OSE_RX_TIMEOUT_M, RC_RX_TIMEOUT_US);
								int16_t state = ose_rxenable(OSE_START_RX_DELAYED, 0);
						...
						case Twr_Tx_Poll_Sent:
							{
//								ose_settrxdelay(OSE_RX_TIMEOUT_M, 1000);
//								int16_t state = ose_rxenable(OSE_START_RX_IMMEDIATE, 0);
								pTwrInfo->testAppState = TA_RX_WAIT_DATA;					//必须优于ose_rxenable
								tag_received_timeout_set(pTwrInfo, false); 
								port_tx_msg("Twr_Tx_Poll_Sent\n", strlen("Twr_Tx_Poll_Sent\n"));
							}
							break;

...
void HalKey_IO_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = HAL_KEY1_PIN|HAL_KEY5_PIN|HAL_KEY6_PIN|HAL_KEY7_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
	GPIO_Init(HAL_KEY1_PORT, &GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin = HAL_KEY2_PIN|HAL_KEY3_PIN|HAL_KEY4_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
	GPIO_Init(HAL_KEY2_PORT, &GPIO_InitStructure);
}


void HalKey_Exit_Init(bool state)
{
	...
	GPIO_EXTILineConfig(HAL_KEY5_PORT_SOURCE, HAL_KEY5_PIN_SOURCE);
	EXTI_InitStructure.EXTI_Line = HAL_KEY5_EXTI_LIN;/*中断线*/
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;/*触发模式*/
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;/*触发信号*/
	EXTI_InitStructure.EXTI_LineCmd = state;/*使能中断线*/
	EXTI_Init(&EXTI_InitStructure);/*调用库函数，初始化EXTI*/
	
	GPIO_EXTILineConfig(HAL_KEY6_PORT_SOURCE, HAL_KEY6_PIN_SOURCE);
	EXTI_InitStructure.EXTI_Line = HAL_KEY6_EXTI_LIN;/*中断线*/
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;/*触发模式*/
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;/*触发信号*/
	EXTI_InitStructure.EXTI_LineCmd = state;/*使能中断线*/
	EXTI_Init(&EXTI_InitStructure);/*调用库函数，初始化EXTI*/
	
	GPIO_EXTILineConfig(HAL_KEY7_PORT_SOURCE, HAL_KEY7_PIN_SOURCE);
	EXTI_InitStructure.EXTI_Line = HAL_KEY7_EXTI_LIN;/*中断线*/
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;/*触发模式*/
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;/*触发信号*/
	EXTI_InitStructure.EXTI_LineCmd = state;/*使能中断线*/
	EXTI_Init(&EXTI_InitStructure);/*调用库函数，初始化EXTI*/
...
void HalKey_NVIC_Config(bool state)
{
	...
	NVIC_InitStructure.NVIC_IRQChannel = HAL_KEY4_EXIT_IRQN;/*配置选中的中断向量*/
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 10;/*配置抢占优先级*/
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;/*配置响应优先级*/
	NVIC_InitStructure.NVIC_IRQChannelCmd = state;/*使能中断向量*/
	NVIC_Init(&NVIC_InitStructure);/*调用库函数，初始化中断向量*/
	
	NVIC_InitStructure.NVIC_IRQChannel = HAL_KEY5_EXIT_IRQN;/*配置选中的中断向量*/
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 10;/*配置抢占优先级*/
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;/*配置响应优先级*/
	NVIC_InitStructure.NVIC_IRQChannelCmd = state;/*使能中断向量*/
	NVIC_Init(&NVIC_InitStructure);/*调用库函数，初始化中断向量*/
	
	NVIC_InitStructure.NVIC_IRQChannel = HAL_KEY6_EXIT_IRQN;/*配置选中的中断向量*/
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 10;/*配置抢占优先级*/
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;/*配置响应优先级*/
	NVIC_InitStructure.NVIC_IRQChannelCmd = state;/*使能中断向量*/
	NVIC_Init(&NVIC_InitStructure);/*调用库函数，初始化中断向量*/
	
	NVIC_InitStructure.NVIC_IRQChannel = HAL_KEY7_EXIT_IRQN;/*配置选中的中断向量*/
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 10;/*配置抢占优先级*/
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;/*配置响应优先级*/
	NVIC_InitStructure.NVIC_IRQChannelCmd = state;/*使能中断向量*/
	NVIC_Init(&NVIC_InitStructure);/*调用库函数，初始化中断向量*/
...
void HalKey_IT_Enable(void)
{
	HalKey_Exit_Init(ENABLE);
	HalKey_NVIC_Config(ENABLE);
}

void HalKey_IT_Disable(void)
{
	HalKey_Exit_Init(DISABLE);
	HalKey_NVIC_Config(DISABLE);
}
...
void Key_Scan(uint32_t timer)
{
	...
	switch(hal_key.mode)
	{
		...
		{
			_dbg_printf("error pressed:%d\n", hal_key.loose_timer - hal_key.press_timer);
